...
def refresh_data():
    keywords = [kw.strip() for kw in st.session_state.keywords_input.split(',') if kw.strip()]
...
st.session_state.keywords_input = st.sidebar.text_input("Keywords (comma-separated)", os.getenv("DEFAULT_KEYWORDS", "aptamer,GPCR,prostate"))
...
st.write(f"Current Keywords: {st.session_state.keywords_input}")
...
if st.session_state.collaborators:
    for collab in st.session_state.collaborators:
        with st.expander(f"{collab['name']} (Status: {collab['status']}"):
